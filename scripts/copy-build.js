...
const getFirebotScriptsFolderPath = () => {
    ...
    if (process.platform === "win32") {
        ...
        appDataFolderPath = path.join(process.env.HOME, "/Library/Application Support");
    ...
    const firebotGlobalSettings = require(path.join(firebotDataFolderPath, "global-settings.json"));
    ...
    const scriptsFolderPath = path.join(firebotDataFolderPath, `/profiles/${activeProfile}/scripts/`);
...
const main = async () => {
    ...
    const destScriptFilePath = path.join(firebotScriptsFolderPath, `${scriptName}`);
