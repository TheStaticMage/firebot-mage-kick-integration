import type { FirebotCustomReward, KickRewardManagementData } from "../../shared/types";
...
describe("KickRewardsBackend", () => {
    ...
    describe("kick:manage-reward", () => {
        it("creates reward and saves management data", async () => {
            ...
            expect(mockRewardsManager.createReward).toHaveBeenCalledWith(firebotReward.twitchData, expect.objectContaining({ cost: 100, skipQueue: false, enabled: true }));
            ...
            expect(mockFrontendCommunicator.send).toHaveBeenCalledWith("kick:reward-management-state-updated", expect.any(Object));
        });
        ...
        it("creates reward with overrides", async () => {
            ...
            expect(mockRewardsManager.createReward).toHaveBeenCalledWith(firebotReward.twitchData, expect.objectContaining({ cost: 500, skipQueue: true, enabled: false }));
        });
        ...
        it("throws error when Firebot reward not found", async () => {
            ...
            expect(mockFrontendCommunicator.send).toHaveBeenCalledWith("kick:reward-management-error", expect.any(Object));
        });
    ...
    describe("kick:refresh-kick-rewards", () => {
        it("returns rewards from API", async () => {
            const mockRewards = [createMockKickReward({ id: "kick-1", title: "Reward 1", cost: 100 }), createMockKickReward({ id: "kick-2", title: "Reward 2", cost: 200 })];
    ...
    describe("kick:resync-reward", () => {
        it("successfully re-syncs a managed reward", async () => {
            ...
            expect(mockRewardsManager.deleteReward).toHaveBeenCalledWith("kick-123");
            expect(mockRewardsManager.createReward).toHaveBeenCalledWith(firebotReward.twitchData, managementData.overrides);
            expect(mockKickRewardsState.setManagementData).toHaveBeenCalledWith("fb-reward-1", expect.objectContaining({ kickRewardId: "kick-new-456" }));
            expect(mockFrontendCommunicator.send).toHaveBeenCalledWith("kick:reward-management-state-updated", expect.any(Object));
        });
        ...
        it("updates reward title if changed", async () => {
            ...
            mockReflectEvent.mockResolvedValue([firebotReward]);
            mockRewardsManager.deleteReward.mockResolvedValue(true);
            mockRewardsManager.createReward.mockResolvedValue(
                createMockKickReward({
                    id: "kick-new-456",
                    title: "New Title",
                    cost: 200
            ...
            expect(mockKickRewardsState.setManagementData).toHaveBeenCalledWith("fb-reward-1", expect.objectContaining({ firebotRewardTitle: "New Title" }));
        });
    ...
    describe("kick:sync-all-rewards", () => {
        it("successfully syncs all managed rewards", async () => {
            ...
            const firebotReward2 = createMockFirebotReward({
                id: "fb-reward-2",
                title: "Reward 2",
                isEnabled: true
            });
            ...
            const kickReward1 = createMockKickReward({
                id: "kick-123",
                title: "Old Title 1",
                cost: 100
            });
            const kickReward2 = createMockKickReward({
                id: "kick-456",
                title: "Reward 2",
                cost: 150
            });
            ...
            expect(mockRewardsManager.updateReward).toHaveBeenCalledWith("kick-123", firebotReward1.twitchData, { cost: 100, skipQueue: false, enabled: true, paused: false });
            expect(mockRewardsManager.updateReward).toHaveBeenCalledWith("kick-456", firebotReward2.twitchData, { cost: 200, skipQueue: true, enabled: true, paused: false });
        ...
        it("counts failed syncs separately", async () => {
            ...
            mockReflectEvent.mockResolvedValue([firebotReward1, firebotReward2]);
            mockRewardsManager.getAllRewards.mockResolvedValue([kickReward1, kickReward2]);
            mockRewardsManager.updateReward.mockResolvedValueOnce(true).mockResolvedValueOnce(false);
        ...
        it("uses Firebot paused state when syncing", async () => {
            ...
            expect(mockRewardsManager.updateReward).toHaveBeenCalledWith("kick-123", firebotReward1.twitchData, { cost: 100, skipQueue: false, enabled: true, paused: true });
        });
    });
    ...
    describe("kick:reconcile-rewards", () => {
        it("re-creates missing Kick rewards", async () => {
            ...
            const newKickReward = createMockKickReward({
                id: "kick-new",
                title: "Test Reward",
                cost: 100
            });
            ...
            expect(mockRewardsManager.createReward).toHaveBeenCalledWith(firebotReward.twitchData, managementState["fb-reward-1"].overrides);
            expect(mockKickRewardsState.setManagementData).toHaveBeenCalledWith("fb-reward-1", expect.objectContaining({ kickRewardId: "kick-new" }));
        ...
        it("updates existing Kick rewards", async () => {
            ...
            expect(mockRewardsManager.updateReward).toHaveBeenCalledWith("kick-123", firebotReward.twitchData, managementState["fb-reward-1"].overrides);
            expect(mockKickRewardsState.setManagementData).toHaveBeenCalledWith("fb-reward-1", expect.objectContaining({ firebotRewardTitle: "Test Reward" }));
            expect(result).toEqual({ updated: 1 });
        });
