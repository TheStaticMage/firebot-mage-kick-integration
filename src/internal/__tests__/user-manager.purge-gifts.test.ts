    ...
    it("removes only expired gifts from a gifter", async () => {
        ...
        await inMemoryGiftDb.insertAsync({
            ...
            gifts: [
                {
                    _id: gifteeId1,
                    sub: { createdAt: createdAt1, expiresAt: expiresAt1 }
                },
                {
                    _id: gifteeId2,
                    sub: { createdAt: createdAt2, expiresAt: expiresAt2 }
                }
            ],
            totalSubs: 2
        });
    ...
    it("removes expired gifts for multiple gifters and leaves all records", async () => {
        ...
        await inMemoryGiftDb.insertAsync({
            ...
            gifts: [
                {
                    _id: gifteeId3,
                    sub: { createdAt: createdAt3, expiresAt: expiresAt3 }
                },
                {
                    _id: gifteeId4,
                    sub: { createdAt: createdAt4, expiresAt: expiresAt4 }
                }
            ],
            totalSubs: 2
        });
