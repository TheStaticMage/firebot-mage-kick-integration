...
describe("httpCallWithTimeout", () => {
    ...
    it("throws error after too many redirects", async () => {
        ...
        await expect(httpCallWithTimeout({ url: "http://test", method: "GET" })).rejects.toThrow(/Too many redirects \(11\)/);
    });
    ...
    it("throws error when redirect response has no url", async () => {
        ...
        await expect(httpCallWithTimeout({ url: "http://test", method: "GET" })).rejects.toThrow(/Redirect response missing Location header/);
    });
    ...
    it("aborts on timeout", async () => {
        ...
        const promise = httpCallWithTimeout({
            url: "http://test",
            method: "GET",
            timeout: 10
        });
    ...
    it("aborts if signal is already aborted", async () => {
        ...
        ).rejects.toThrow("API request aborted");
    });
    ...
    it("retries multiple times on 429 with exponential backoff", async () => {
        ...
        global.setTimeout = ((fn: (...args: any[]) => void, delay: number) => {
            if (delay > 0 && delay < 10000) {
                // Filter out timeout delays, only capture backoff delays
    ...
    it("throws timeout error when 429 retries would exceed effectiveTimeout", async () => {
        ...
        try {
            ...
            expect(logger.warn).toHaveBeenCalledWith(expect.stringContaining("Rate limit retry would exceed timeout for http://test"));
        } finally {
            global.setTimeout = originalSetTimeout;
