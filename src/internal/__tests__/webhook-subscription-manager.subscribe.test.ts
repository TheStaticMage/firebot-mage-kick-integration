...
describe("WebhookSubscriptionManager.initialize", () => {
    ...
    it("creates subscriptions if needed", async () => {
        ...
        expect(kick.httpCallWithTimeout).toHaveBeenCalledWith("/public/v1/events/subscriptions", "POST", expect.stringContaining("chat.message.sent"));
    });
    ...
    it("deletes subscriptions if needed", async () => {
        ...
        kick.httpCallWithTimeout.mockResolvedValue({});
        await expect(manager.initialize()).resolves.toBeUndefined();
        expect(kick.httpCallWithTimeout).toHaveBeenCalledWith("/public/v1/events/subscriptions?id=sub1&id=sub2", "DELETE");
    });
