...
let httpRequestQueue: Promise<unknown> = Promise.resolve();

export function resetHttpRequestQueueForTests(): void {
    httpRequestQueue = Promise.resolve();
}
...
export async function httpCallWithTimeout<T = any>(req: HttpCallRequest): Promise<T> {
        ...
        while (true) {
            try {
                ...
                if (response.status === 429) {
                    ...
                    if (retryAfter) {
                        logger.debug(`Rate limit Retry-After header for ${currentUrl}: ${retryAfter}`);
                    } else {
                        const headerEntries = Object.fromEntries(response.headers?.entries?.() ?? []);
                        logger.debug(`No Retry-After header present for ${currentUrl}: ${JSON.stringify(headerEntries)}`);
                    }
    ...
    const queuedRequest = httpRequestQueue.catch(() => undefined).then(execute);
    httpRequestQueue = queuedRequest.catch(() => undefined);
