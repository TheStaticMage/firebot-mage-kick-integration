...
describe("moderatorTimeoutEffect.onTriggerEvent", () => {
    ...
    it("successfully times out a user with valid parameters", async () => {
        ...
        const result = await moderatorTimeoutEffect.onTriggerEvent({
            trigger,
            effect
        } as any);
    ...
    it("returns false and logs error when API call fails", async () => {
        ...
        expect(integration.kick.userApi.banUserByUsername).toHaveBeenCalledWith("testuser", 5, true, "Test reason");
