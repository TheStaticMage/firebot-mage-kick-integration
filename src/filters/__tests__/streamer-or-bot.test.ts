...
describe("streamerOrBotFilter.predicate", () => {
    ...
    it("returns true for Kick streamer (is)", async () => {
        const eventData = {
            ...baseEventData,
            eventSourceId: IntegrationConstants.INTEGRATION_ID,
            eventMeta: { username: "StreamerKick" }
        };
    ...
    it("returns false for Kick not streamer (is)", async () => {
        const eventData = {
            ...baseEventData,
            eventSourceId: IntegrationConstants.INTEGRATION_ID,
            eventMeta: { username: "NotAStreamer" }
        };
        const result = await streamerOrBotFilter.predicate({ comparisonType: "is", value: "streamer" }, eventData);
        expect(result).toBe(false);
    });
    ...
    it("returns true for Twitch streamer", async () => {
        const eventData = {
            ...baseEventData,
            eventSourceId: "twitch",
            eventMeta: { username: "StreamerTwitch" }
        };
        const result = await streamerOrBotFilter.predicate({ comparisonType: "is", value: "streamer" }, eventData);
        expect(result).toBe(true);
    });
...
describe("streamerOrBotFilter.events", () => {
    it('should contain both twitch and INTEGRATION_ID "chat-message" events', () => {
        const twitchEvent = streamerOrBotFilter.events.find((e) => e.eventSourceId === "twitch" && e.eventId === "chat-message");
        const integrationEvent = streamerOrBotFilter.events.find((e) => e.eventSourceId === IntegrationConstants.INTEGRATION_ID && e.eventId === "chat-message");
    ...
    it('returns correct display value for "streamer"', () => {
        expect(
            getSelectedValueDisplay({
                comparisonType: dummyComparisonType,
                value: "streamer"
            })
        ).toBe("Streamer");
    });
