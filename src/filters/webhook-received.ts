import type { EventData, EventFilter, PresetValue } from "@crowbartools/firebot-custom-scripts-types/types/modules/event-filter-manager";
    ...
    presetValues(): PresetValue[] {
        return [
            ...
            { value: "channel.followed", display: "channel.followed" },
            {
                ...
                display: "channel.subscription.renewal"
            },
            {
                value: "channel.subscription.gifts",
                display: "channel.subscription.gifts"
            },
            {
                value: "channel.subscription.new",
                display: "channel.subscription.new"
            },
            {
                value: "livestream.metadata.updated",
                display: "livestream.metadata.updated"
            },
            {
                value: "livestream.status.updated",
                display: "livestream.status.updated"
            },
...
export const webhookReceivedLatencyFilter: EventFilter = {
    ...
    comparisonTypes: [ComparisonType.LESS_THAN, ComparisonType.LESS_THAN_OR_EQUAL_TO, ComparisonType.GREATER_THAN, ComparisonType.GREATER_THAN_OR_EQUAL_TO],
    valueType: "number",
    predicate: async (filterSettings, eventData: EventData): Promise<boolean> => {
        const { comparisonType, value } = filterSettings;
