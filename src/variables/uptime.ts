    ...
    evaluator: async (trigger, username) => {
        try {
            ...
            if (!channel.stream.startTime) {
                logger.debug(`[${IntegrationConstants.INTEGRATION_ID}] No start time found for channel: ${username}`);
                return 0;
            }
            ...
            logger.error(`[${IntegrationConstants.INTEGRATION_ID}] Error retrieving uptime for username ${username}: ${error}`);
            return 0;
        }
    }
};
