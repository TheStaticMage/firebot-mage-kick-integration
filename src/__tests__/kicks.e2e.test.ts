...
describe("e2e kicks gifted events", () => {
    ...
    describe("webhook only - kicks.gifted", () => {
        it("handles webhook kicks gifted event", async () => {
            ...
            expect(triggerEventMock).toHaveBeenCalledWith(IntegrationConstants.INTEGRATION_ID, "kicks-gifted", expectedKickMetadata);

            // Should NOT trigger Twitch cheer event since it's disabled
            expect(triggerEventMock).not.toHaveBeenCalledWith("twitch", "cheer", expect.any(Object));
        });
        ...
        it("handles webhook kicks gifted event with twitch forwarding enabled", async () => {
            ...
            expect(triggerEventMock).toHaveBeenCalledWith("twitch", "cheer", expectedKickMetadata);
        });
    });
});
