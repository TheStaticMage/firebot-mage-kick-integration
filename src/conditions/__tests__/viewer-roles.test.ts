...
describe("viewerRolesCondition.predicate", () => {
    ...
    describe("comparison types", () => {
        ...
        describe("unknown comparison types", () => {
            it("returns false and logs warning for unknown comparison type", async () => {
                ...
                expect(result).toBe(false);
                expect(mockLogger.warn).toHaveBeenCalledWith("viewerroles condition: Unknown comparison type: unknown_comparison");
            });
        });
    });
    ...
    describe("logging", () => {
        it("logs debug message when user has role", async () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith("viewerroles condition: Viewer testuser (kick) has role mod");
        });
        ...
        it("logs debug message when user does not have role", async () => {
            ...
            expect(mockLogger.debug).toHaveBeenCalledWith("viewerroles condition: Viewer testuser (twitch) does NOT have role vip");
        });
    });
    ...
    describe("integration with RoleManager", () => {
        ...
        it("handles RoleManager errors gracefully", async () => {
            ...
            await expect(viewerRolesCondition.predicate(conditionSettings, trigger)).rejects.toThrow("Role check failed");
        });
    });
});
